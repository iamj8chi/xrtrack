...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!deviceInfo.supportsWebGL) {
        ...
        document.querySelector('#error').innerHTML = '<p>Your device does not support WebGL, which is required for AR.</p>';
    ...
    Utils.checkARSupport().then(supported => {
        if (!supported) {
            ...
            document.querySelector('#error').innerHTML = '<p>Camera access is required for AR. Please allow camera permissions and refresh.</p>';
            document.querySelector('#error').style.display = 'flex';
            return;
        }
